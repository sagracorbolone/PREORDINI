...
# AggiornaListino_final.py
# - Categorie in ordine naturale (no ORDER BY)
# - Pietanze ordinabili per categoria con campo "Ord." (riparte da 1) e checkbox "Includi"
# - Persistenza tra sessioni in ordine_pietanze.json (accanto allo script)
# - data.js generato già completo di initData(), pronto per l'app (index.html chiama initData())
# - GUI scrollabile con mouse wheel/trackpad e tasti PgUp/PgDn/Home/End
#
# Adatta OUTPUT_DATA_JS al tuo percorso se diverso.
...
def aggiorna_listino():
    ...
    try:
        ...
        # Topbar (filtro)
        topbar = ttk.Frame(root); topbar.pack(fill="x", padx=10, pady=6)
        ...
        scroll = ScrollableFrame(root); scroll.pack(fill="both", expand=True)
        inner = scroll.inner
        ...
        for cat in categorie:
            ...
            for idx, p in enumerate(cat_items):
                ...
                ttk.Checkbutton(rowf, variable=var_chk).grid(row=0, column=0, sticky="w", padx=(0,6))
                ...
                ttk.Entry(rowf, textvariable=var_ord, width=6, justify="right").grid(row=0, column=1, sticky="w", padx=(0,10))
        ...
        def apply_filter(*args):
            ...
            for cat_id, rows in per_cat_widgets.items():
                for r in rows:
                    ...
                    try:
                        if visible: r['rowf'].pack(fill="x", padx=8, pady=1)
                        else: r['rowf'].pack_forget()
        ...
        # Bottom bar
        bottombar = ttk.Frame(root); bottombar.pack(fill="x", padx=10, pady=8)
        ...
        def on_continua():
            ...
            # Genera data.js con initData()
            js = []
            js.append(f"// data.js - Generato automaticamente (Data: {datetime.now().isoformat()})")
            js.append("")
            js.append("var elencoPrincipale = " + json.dumps(elenco_principale, indent=2, ensure_ascii=False) + ";")
            js.append("var categorie = " + json.dumps(categorie, indent=2, ensure_ascii=False) + ";")
            js.append("var elencoPietanze = " + json.dumps(filtered_elenco_pietanze, indent=2, ensure_ascii=False) + ";")
            js.append("")
            js.append("// Inizializzazione per l'app: index.html chiama initData()")
            js.append("function initData(){")
            js.append("  try {")
            js.append("    if (typeof Data === 'function') { window.dataManager = new Data(); }")
            js.append("    window.elencoPrincipale = elencoPrincipale;")
            js.append("    window.categorie = categorie;")
            js.append("    window.elencoPietanze = elencoPietanze;")
            js.append("  } catch(e) { console.error('Errore in initData():', e); }")
            js.append("}")
            js_content = "\n".join(js)
        ...
        print('ERRORE DB:', e)
        try: messagebox.showerror("Errore Database", f"Errore DB:\n{e}")
        except Exception: pass
    except Exception as e:
        print('ERRORE:', e)
        try: messagebox.showerror("Errore", f"Errore:\n{e}")
        except Exception: pass
    finally:
        if conn:
            conn.close()
